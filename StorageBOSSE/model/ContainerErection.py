...
class ContainerErection(CostModule):
    """
    Cost of erecting the battery+inverter+transformer (BESS) container once the concrete pad
    has been constructed and is cured. This module calculates the cost of
    erecting the containers using 20-40 ton cranes.

    This module assumes that it takes 2 hours to offload each BESS
    container and place on it's respective concrete pad.

    """

    def __init__(self, input_dict, output_dict, project_name):
        super(ContainerErection, self).__init__(input_dict, output_dict, project_name)
    ...
    def run_module(self):
        try:
            ...
            traceback.print_exc()
            print(f"Fail {self.project_name} ContainerErection")
            self.input_dict['error']['ContainerErection'] = error
